...
Almost most applications would have somebody (a prover) claiming they have some email with some speific information. The role of the smart contract is to host the verification logic (at simplest, verify the rsa signature) and ask the user to submit their full eml file to be processed by the smart contract. If it verifies, then the smart contract logic can base an action (eg.release of funds, or minting an ownership NFT) based on it. 
...
Our smart contract logic is now complete. We have a state variable representing the public key hash to compare against, and also have a `verify_handle` smart contract method we can call from a mina transaction. Next step we need to create the fronend for the user and what exactly shoule we process the eml files. 
