...
class TwitterVerify extends SmartContract {
  @state(Field) TwitterPublicKeyHash = State<Field>();

  @method async initTwitter(initialPublicKeyHash: Field) {
    super.init();
    
    // Initialize TwitterPublicKeyHash with the provided value
    this.TwitterPublicKeyHash.set(initialPublicKeyHash);
  }
  ...
  @method async verify_handle(
    ...
    // email verify email signature without body hash check
    emailVerify(
        paddedHeader,
        headerHashIndex,
        signature,
...
// const zkAppInstance = new TwitterVerify(zkAppAddress);
