...
export { pkcs1v15Encode, canonicalizeBody };
 ...
 */
function pkcs1v15Encode(digest: Bytes, _emLen: number) {
  // SHA256 algorithm identifier
  const digestAlgorithm = Bytes(19).fromHex(
  ...
  if (_emLen < digest.length + digestAlgorithm.length + 11) {
    throw new Error(
      `Selected hash algorithm has a too long digest (${
        digest.length + digestAlgorithm.length
      } bytes).`
    );
  }
...
function canonicalizeBody(body: string): Buffer {
  ...
  const normalizedBuffer = Buffer.from(
    normalizedStr.replace(/\r?\n/g, '\r\n'),
    'utf-8'
  );
  // Step 4: Add CRLF sequence as per RFC
  const canonicalizedBody = Buffer.concat([
    normalizedBuffer,
    Buffer.from('\r\n', 'utf-8'),
  ]);
